...
---@field lines string[]|BufferSection If a buffer section is provided, it is assumed to be empty

---@class BufferSectionParams
...
---@field child_sections BufferSection Linked list of child sections
---@field next_section BufferSection|nil
---@field private _on_lines_changed fun(line_delta: number)
---@field private _buf_id number|nil
local BufferSection = {}
BufferSection.__index = BufferSection

---@param params BufferSectionParams
...
---@param rel_line_edit LineEdit Section relative line edit
function BufferSection:edit_lines(rel_line_edit)
	...
	if rel_line_edit.start_row > self.row_len then
		...
		error(string.format("Invalid start line: %d", rel_line_edit.start_row))
	end

	if rel_line_edit.row_end == -1 then
		rel_line_edit.row_end = self:get_end_row()
	end

	local start_row = self.start_row + rel_line_edit.start_row
	local end_row = self.start_row + rel_line_edit.row_end
	...
	if getmetatable(rel_line_edit.lines) == BufferSection then
		...
		buffer_section.start_row = self.start_row + rel_line_edit.start_row
	...
	---@type number
	local num_deleted_lines = math.max(0, rel_line_edit.row_end - 1 - rel_line_edit.start_row)
	---@type number
	local num_added_lines = #lines
	---@type number
	local len_delta = num_added_lines - num_deleted_lines
	if rel_line_edit.start_row == rel_line_edit.row_end then
		len_delta = #lines
	end
...
---@param row_end number 0-indexed, buffer-relative
---@return BufferSection[]
function BufferSection:_find_overlapping_child_sections(start_row, row_end)
	---@type BufferSection[]
	local child_sections = {}
	local buffer_section = self.child_sections
	while buffer_section ~= nil do
		if buffer_section:contains_line(start_row) or buffer_section:contains_line(row_end) then
...
---@private
---@param child_section BufferSection
