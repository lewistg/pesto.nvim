...
function BuildEventsBuffer:new(build_event_tree, build_event_file_loader)
  ...
  local failure_bazel_target_results, successful_bazel_target_results =
    o:_load_events(build_event_tree)
...
function BuildEventsBuffer:_get_failed_action_logs(target_completed_event)
  assert(
    target_completed_event.id.target_completed ~= nil,
    'Build event does not have a TargetCompletedId id'
  )
