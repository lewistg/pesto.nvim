...
local function get_comp_words(bash_command_tokens, cursor_pos)
  ...
  local _, current_token_index =
    bash_command_util.find_current_token(bash_command_tokens, cursor_pos)
...
function M.get_bazel_bash_completion_request(nvim_complete_opts, bash_command_tokens)
  ...
  table.insert(
    completion_request,
    bash_completion_args.comp_point(tostring(nvim_complete_opts.cursor_pos))
  )
