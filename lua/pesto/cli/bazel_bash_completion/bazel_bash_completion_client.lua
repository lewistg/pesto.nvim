...
function BazelBashCompletionClient:get_completions(options)
  ...
  if self._bash_completion_server_system_object == nil then
    ...
    logger.info(
      string.format(
        'Started bash completion server. pid=%d',
        self._bash_completion_server_system_object.pid
      )
    )
  end
  ...
  if self._bash_completion_server_system_object ~= nil then
    logger.trace(
      'killing completion server after timeout pid: '
        .. tostring(self._bash_completion_server_system_object.pid)
    )
...
function BazelBashCompletionClient:_start_completion_server()
  ...
  return vim.system(server_command, {
    ...
    vim.schedule(function()
      logger.info(
        string.format('bash completion server exited with code %d', system_completed.code)
      )
    end)
    self._current_response_line_handler = nil
  end)
...
function BazelBashCompletionClient:_get_bash_completion_script()
  if self._bash_completion_script == nil then
    ...
    logger.trace(
      string.format('bash completion search paths: %s', table.concat(bash_completion_scripts, ','))
    )
