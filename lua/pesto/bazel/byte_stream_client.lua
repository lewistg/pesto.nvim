...
function ByteStreamClient:are_remote_apis_helpers_installed()
  if not self._remote_apis_helpers_installed then
    ...
    local is_installed_command =
      self._remote_apis_helpers_command_builder:get_is_installed_command()
    local result = vim
      .system(is_installed_command, {
        clear_env = true,
      })
      :wait()
    logger.info(
      string.format(
        "is_installed_command='%s', result=%d",
        table.concat(is_installed_command, ' '),
        result.code
      )
    )
  ...
  local job_id = vim.fn.jobstart(command, {
    ...
    on_exit = function(_, exit_code)
      ...
      if exit_code ~= 0 then
        ...
        logger.error(
          string.format(
            'download command failed. exit_code=%d command="%s"',
            exit_code,
            command_str
          )
        )
